...
int HeadConductionSolver::Gradient( double *phi, double *Bphif, double **phigd )
{
    int    i,g, c1,c2;
    double lambda,pf;
    // using Gauss theorem
...
int HeadConductionSolver::Limiter_MLP(double UC[],double **GradU)
{
    int    i,j,iv;
    ...
    for( i=0; i<Ncel; i++ )
    {
        for(j=0; j<8; j++ ) // this may be a little wasteful, optimize later
        {
            ...
            UMax[iv]= MAX( UMax[iv], UC[i] );
            UMin[iv]= MIN( UMin[iv], UC[i] );
        }
    }
    ...
    for( i=0; i<Ncel; i++ )
    {
        ...
        for(j=0; j<8; j++ ) // this may be a little wasteful, 8 changed to more 
        {
			...
			// Barth
			fi = MIN( fi, Barth_fun(us,umin,umax) );
			// Venkatanishnan
			// fi = MIN( fi, smoothfun( us,umin,umax, 1./ Cell[i].vol**3 ) );
        }
        ficell[i]= fi;
    }
...
int HeadConductionSolver::Limiter_Barth(double UC[],double **GradU)
{
    int    i,j,iv,in;
...
int HeadConductionSolver::Limiter_WENO(double *UC,double **GradU)
{
    int i,j, ic,iface;
    double ss,wei, **gdTmp;
